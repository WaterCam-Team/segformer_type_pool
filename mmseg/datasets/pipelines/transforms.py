...
# mmcv-style augmentations for SegFormer
...
class WaterAugMMCV:
    """
    Photometric distortion in mmcv style with extra water-case modes.
    Expects results dict with 'img' (np.uint8, BGR). Returns updated dict.
    """
    def __init__(self,
                 brightness_delta=32,
                 contrast_range=(0.5, 1.5),
    ...
    def saturation(self, img):
        if random.randint(2):
            hsv = mmcv.bgr2hsv(img)
            hsv[..., 1] = self.convert(hsv[..., 1],
                                       alpha=random.uniform(self.saturation_lower, self.saturation_upper))
            return mmcv.hsv2bgr(hsv)
        return img
    ...
    # extra water-case modes
    def _glare(self, img):
        img = self.convert(img, alpha=random.uniform(1.05, 1.25), beta=random.uniform(5, 25))
        # mild gamma brighten: approximate via HSV V-channel
        hsv = mmcv.bgr2hsv(img); v = hsv[...,2].astype(np.float32)
        v = np.clip(255.0 * np.power(v/255.0, random.uniform(0.7, 0.9)), 0, 255).astype(np.uint8)
        hsv[...,2] = v
        return mmcv.hsv2bgr(hsv)

    def _night(self, img):
        img = self.convert(img, alpha=random.uniform(0.6, 0.9), beta=random.uniform(-15, 10))
        hsv = mmcv.bgr2hsv(img)
        hsv[...,1] = self.convert(hsv[...,1], alpha=random.uniform(0.6, 0.9))  # desaturate
        hsv[...,2] = self.convert(hsv[...,2], alpha=random.uniform(0.6, 0.85)) # darken
        return mmcv.hsv2bgr(hsv)

    def _muddy(self, img):
        hsv = mmcv.bgr2hsv(img)
        # bump hue toward yellow/brown, reduce saturation a bit
        hsv[...,0] = (hsv[...,0].astype(int) + random.randint(3, 10)) % 180
        hsv[...,1] = self.convert(hsv[...,1], alpha=random.uniform(0.7, 0.95))
        return mmcv.hsv2bgr(hsv)

    def _reflection(self, img):
        # subtle hue wiggle + mild contrast up
        img = self.convert(img, alpha=random.uniform(1.05, 1.15))
        hsv = mmcv.bgr2hsv(img)
        hsv[...,0] = (hsv[...,0].astype(int) + random.randint(-3, 3)) % 180
        hsv[...,2] = self.convert(hsv[...,2], alpha=random.uniform(0.95, 1.05))
        return mmcv.hsv2bgr(hsv)

    def __call__(self, results):
        img = results['img']

        # base photometric sequence (brightness -> sat/hue -> contrast in random order)
        # img = self.brightness(img)
        # mode = random.randint(2)
        # if mode == 1:
        #     img = self.contrast(img)
    ...
    def __repr__(self):
        return (f"WaterAugMMCV(brightness_delta={self.brightness_delta}, "
                f"contrast_range=({self.contrast_lower},{self.contrast_upper}), "
                f"saturation_range=({self.saturation_lower},{self.saturation_upper}), "
                f"hue_delta={self.hue_delta}, "
                f"p_glare={self.p_glare}, p_night={self.p_night}, "
                f"p_muddy={self.p_muddy}, p_reflection={self.p_reflection})")
