...
def eval_metrics(results,
    ...
    #     if results[i].shape != gt_seg_maps[i].shape:
    #         h,w = gt_seg_maps[i].shape
    #         results[i] = cv2.resize(results[i].astype(float),(w,h))
            #results[i] = resize(results[i], size=gt_seg_maps[i].size(),mode='bilinear',align_corners=False)
