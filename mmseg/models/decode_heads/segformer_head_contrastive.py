...
class SegFormerHead_contrastive(BaseDecodeHead):
    """
    SegFormer: Simple and Efficient Design for Semantic Segmentation with Transformers
    """
    def __init__(self, feature_strides, use_text=False,use_yoloe=False,use_contras=False,**kwargs):
        super(SegFormerHead_contrastive, self).__init__(input_transform='multiple_select', **kwargs)
        assert len(feature_strides) == len(self.in_channels)
        assert min(feature_strides) == feature_strides[0]
        self.feature_strides = feature_strides
        self.use_text = use_text
        self.use_contras = use_contras
        ...
        if self.use_text and self.use_yoloe:
            self.reprta_head = RepRTAHead(in_ch=embedding_dim, clip_dim=512, mid_ch=256)
    ...
    def contrastive_loss(self, F_visual, seg_label, Ew):
        ...
        #     mode='bilinear',
        #     align_corners=False)
        seg_label = F.interpolate(seg_label.float(), size=F_visual.shape[2:], mode="nearest").long()
        seg_label = seg_label.squeeze(1)
        ##### added by Huantao ####
        seg_label = (seg_label > 0).to(torch.long)
        ##########################
        B, D, H, W = F_visual.shape
        F_flat = F_visual.permute(0,2,3,1).reshape(-1, D)   # [N, D]
        seg_label_flat = seg_label.view(-1)    
        
        N = F_flat.shape[0]
        idx = torch.randperm(N, device=F_flat.device)[:N//50]  # take 2%
        F_sub = F_flat[idx]
        y_sub = seg_label_flat[idx]

        # cosine sim with water
        # sim_w = (F_flat * Ew.view(1, -1)).sum(1)          # [N]
        # with torch.cuda.amp.autocast():
        # sim_w = F_flat @ Ew

        # # loss terms
        # pos_mask = (seg_label_flat == 1)
        # neg_mask = (seg_label_flat == 0)

        # loss_pull = (1 - sim_w)[pos_mask].mean()          # pull water pixels closer
        # loss_push = (sim_w.clamp(min=0))[neg_mask].mean() # push bg pixels away
    ...
    def forward(self, inputs, gt_label):
        ...
        if self.use_text and self.use_yoloe:
            ...
            if gt_label!=None:
                ...
                Ew = self.E_WATER / (self.E_WATER.norm() + 1e-6) 
                align_loss = self.masked_pooled_alignment_loss(Fv, Ew, gt_label)
                return x, align_loss  
            return x     
        elif self.use_text:
            F_visual = self.proj_to_clip(_c)                              # [B,512,H,W]
            F_visual = F_visual / (F_visual.norm(dim=1, keepdim=True) + 1e-6)

            Ew = self.E_WATER / (self.E_WATER.norm() + 1e-6)       # [512]
            logit_water = self.logit_scale * (F_visual * Ew.view(1,-1,1,1)).sum(1, keepdim=True)  # [B,1,H,W]
            logit_bg    = self.bg_head(_c)                         # [B,1,H,W]
            x = torch.cat([logit_bg, logit_water], dim=1)
            
            if self.use_contras and gt_label!=None:
                constrastive_loss = self.contrastive_loss(F_visual, gt_label, Ew)
                return x, constrastive_loss
